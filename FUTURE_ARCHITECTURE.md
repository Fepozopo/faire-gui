...
- initial 30-day history synchronization using `updated_at_min`;
...
A connection without a completed bootstrap synchronizes every cursor page beginning at the 30-day `updated_at_min` boundary. A user can enter an earlier **Updated At Minimum** date and refresh to expand retained history; the earlier boundary is saved for future sessions. Rebuilding local data always returns to the 30-day default window.
