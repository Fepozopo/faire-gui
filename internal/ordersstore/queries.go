...
func (s *SQLiteStore) List(ctx context.Context, query ListQuery) (ListPage, error) {
	...
	statement := `SELECT order_id, display_id, state, address_name, total_payout_amount_minor, total_payout_currency, commission_bps, source, created_at_utc, expected_ship_at_utc, updated_at_utc, synced_at_utc
...
func (s *SQLiteStore) FindByDisplayID(ctx context.Context, connectionID, displayID string) (LocalRow, error) {
	row := s.database.QueryRowContext(ctx, `SELECT order_id, display_id, state, address_name, total_payout_amount_minor, total_payout_currency, commission_bps, source, created_at_utc, expected_ship_at_utc, updated_at_utc, synced_at_utc FROM orders WHERE connection_id = ? AND display_id = ?`, connectionID, displayID)
...
// UpsertOrders atomically inserts or replaces every supplied raw projection, including Faire total payout and commission BPS, and complete snapshot without regressing newer versions.
...
func (s *SQLiteStore) UpsertOrders(ctx context.Context, records []OrderRecord) error {
	...
	statement := `INSERT INTO orders (connection_id, order_id, display_id, state, address_name, total_payout_amount_minor, total_payout_currency, commission_bps, source, created_at_utc, expected_ship_at_utc, updated_at_utc, order_snapshot_json, snapshot_schema_version, synced_at_utc)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
...
func scanLocalRow(scanner interface{ Scan(...any) error }) (LocalRow, error) {
	var row LocalRow
	var state, address, payoutCurrency, source sql.NullString
	var totalPayout, commission sql.NullInt64
	var created, expected sql.NullInt64
	var updated, synced int64
	if err := scanner.Scan(&row.OrderID, &row.DisplayID, &state, &address, &totalPayout, &payoutCurrency, &commission, &source, &created, &expected, &updated, &synced); err != nil {
		return LocalRow{}, err
	}
	row.State, row.AddressName, row.TotalPayoutCurrency, row.Source = state.String, address.String, payoutCurrency.String, source.String
...
// nullableInt64 converts an optional integer to a SQLite value or NULL.
...
// nullableInt64Pointer converts a nullable SQLite integer to an optional Go integer.
