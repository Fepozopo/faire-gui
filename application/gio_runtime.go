...
// runWindow handles Gio window events, begins startup work on its first frame, drains background results including Sage fulfillment and local payout exports, and submits complete frames.
...
func (ui *DesktopUI) runWindow() error {
	...
	for {
		...
		case app.FrameEvent:
			...
			ui.drainSageFulfillmentListenerResults()
			ui.drainSageFulfillmentRequests()
			ui.drainSageFulfillmentAcknowledgements()
			ui.drainResults()
			ui.drainBrandIDRefreshResults()
